...
// command to compile
// g++ Inputs.cpp -o Inputs -Wall -O -L/usr/local/lib -lm -lgsl -lgslcblas
...
int i;
	...
	for (i=0; i<nfacets; i++){
		...
		theta = acos(dot(Vinfn_neg, (pfacet+i)->normal));

    if(theta < abs(0.5*M_PI)){
      Sref += (pfacet+i)->area * cos(theta);
    }
    ...
    if(theta == 0){
      ...
      Ct_s = -1.0 * (ST_s * sin(theta)/ s / sqrt(M_PI)) * (exp(-1.0*(s*cos(theta))*(s*cos(theta)) ) + sqrt(M_PI) * s *cos(theta)*(1.0+erf(s*cos(theta))));
      ...
      Ct_ads = -1.0 * (ST_ads * sin(theta)/ s / sqrt(M_PI)) * (exp(-1.0*(s*cos(theta))*(s*cos(theta)) ) + sqrt(M_PI) * s *cos(theta)*(1.0+erf(s*cos(theta))));
    }

    if(theta == 0.5 * M_PI){

       double Cp1_s = exp(-1.0*(s*cos(theta))*(s*cos(theta))) * ((2.0-SN_s)*s*cos(theta)/sqrt(M_PI) + SN_s*sqrt(Tw/Tinf)/2);
       double Cp2_s = (1.0+erf(s*cos(theta))) * ((2.0-SN_s) * ((s*cos(theta))*(s*cos(theta)) + 0.5) + 0.5*SN_s*s*cos(theta)*sqrt(M_PI*Tw/Tinf));
        Cp_s = (Cp1_s+Cp2_s)/(s*s);
        
       double Cp1_ads = exp(-1.0*(s*cos(theta))*(s*cos(theta))) * ((2.0-SN_ads)*s*cos(theta)/sqrt(M_PI) + SN_s*sqrt(Tw/Tinf)/2);
       double Cp2_ads = (1.0+erf(s*cos(theta))) * ((2.0-SN_ads) * ((s*cos(theta))*(s*cos(theta)) + 0.5) + 0.5*SN_ads*s*cos(theta)*sqrt(M_PI*Tw/Tinf));
